...
jest.mock("docusign-esign/src/api/UsersApi", () => {
    return jest.fn().mockImplementation(() => {
        return {
            list: jest.fn().mockImplementation((userApi, args) => {
                return apiResponse;
            })
        }
    });
...
describe("executeRequestThrottleOn", () => {
    ...
    test("[0] no rate limit exhausted", async () => {
        const usersApi = new UsersApi(new ApiClient({ basePath: 'https://fakeapi.acme.org/restapi', oAuthBasePath: '' }));
        const result = await executeRequestThrottleOn(usersApi.list, usersApi, [accountId, {}]);

        expect(usersApi.list).toBeCalled();
        expect(usersApi.list).toBeCalledTimes(1);
        expect(usersApi.list).toHaveBeenCalledWith(accountId, {});
        expect(result.totalSetSize).toStrictEqual("2");
    });

    test("[1] other error", () => {
        __setMockResponseForApiCall(Promise.reject(unauthorizedError));
        const usersApi = new UsersApi(new ApiClient({ basePath: 'https://fakeapi.acme.org/restapi', oAuthBasePath: '' }));

        expect(executeRequestThrottleOn(usersApi.list, usersApi, [accountId, {}]))
            .rejects
